...
def SilverlightCheck(url, level):
  #Log('Checking for silverlight')
  try: Level = HTML.ElementFromURL(url).xpath('//div[@id="Level%d"]' % (level+1))[0]
  except: return False
  for clip in Level.xpath('.//dl[@class="Item"]'):
    ...
    if info['Format'] == 'WMV':
      #Log('Silverlight found')
      return True
    else: #Log('not a silverlight video')
      pass
  return False